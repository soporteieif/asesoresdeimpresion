...
class StAdvancedBannerClass extends ObjectModel
{
	/** @var integer id*/
	public $id;
	/** @var integer group id*/
	public $id_st_advanced_banner_group;
	...
	public static $definition = array(
		'table'     => 'st_advanced_banner',
		'primary'   => 'id_st_advanced_banner',
		'multilang' => true,
		'fields'    => array(
			'id_st_advanced_banner_group' => array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt', 'required' => true),
    ...
    {            
		...
		$res = parent::delete();
        if ($res)
            StAdvancedBannerFontClass::deleteBySlider($this->id);
        return $res;
    }
    
	public static function getAll($id_st_advanced_banner_group, $id_lang, $active=0)
	{
	   if (!Validate::isUnsignedId($id_lang))
		...
		$result = Db::getInstance()->executeS('
			SELECT sms.*, smsl.*
			FROM `'._DB_PREFIX_.'st_advanced_banner` sms
			LEFT JOIN `'._DB_PREFIX_.'st_advanced_banner_lang` smsl ON (sms.`id_st_advanced_banner` = smsl.`id_st_advanced_banner`)
			WHERE smsl.`id_lang` = '.(int)$id_lang.' AND sms.`id_st_advanced_banner_group`='.(int)$id_st_advanced_banner_group.($active ? ' AND sms.`active`=1 ' : '').'
    ...
    public function updatePosition($way, $position)
	{
		if (!$res = Db::getInstance()->executeS('
			SELECT `id_st_advanced_banner`, `position`, `id_st_advanced_banner_group`
			FROM `'._DB_PREFIX_.'st_advanced_banner` 
			WHERE `id_st_advanced_banner_group` = '.(int)$this->id_st_advanced_banner_group.'
			ORDER BY `position` ASC'
		))
			return false;

		foreach ($res as $banner)
			if ((int)$banner['id_st_advanced_banner'] == (int)$this->id)
				$moved_banner = $banner;
		...
		return (Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'st_advanced_banner`
			SET `position`= `position` '.($way ? '- 1' : '+ 1').'
			WHERE `position`
			'.($way
				? '> '.(int)$moved_banner['position'].' AND `position` <= '.(int)$position
				: '< '.(int)$moved_banner['position'].' AND `position` >= '.(int)$position).'
			AND `id_st_advanced_banner_group`='.(int)$moved_banner['id_st_advanced_banner_group'])
		&& Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'st_advanced_banner`
			SET `position` = '.(int)$position.'
			WHERE `id_st_advanced_banner` = '.(int)$moved_banner['id_st_advanced_banner']));
	}
    public function checkPosition()
    {
        $check = Db::getInstance()->getValue('
			SELECT count(0)
			FROM `'._DB_PREFIX_.'st_advanced_banner` 
			WHERE `id_st_advanced_banner_group` = '.(int)$this->id_st_advanced_banner_group.' AND `position`='.(int)$this->position.($this->id ? ' AND `id_st_advanced_banner`!='.$this->id : '')
		);
        if($check)
    ...
    public static function deleteBannersByGroupId($id_st_advanced_banner_group)
    {
        $res = true;
        $banners = Db::getInstance()->executeS('
            SELECT `id_st_advanced_banner`
            FROM `'._DB_PREFIX_.'st_advanced_banner`
            WHERE `id_st_advanced_banner_group`='.(int)$id_st_advanced_banner_group
        );
        $banner_ids = array();
        if(count($banners))
            foreach($banners as $v)
                $banner_ids[] = $v['id_st_advanced_banner'];
        if(count($banner_ids))
        {
            $res &= Db::getInstance()->execute('
    			DELETE FROM `'._DB_PREFIX_.'st_advanced_banner`
    			WHERE `id_st_advanced_banner` in ('.implode(',',$banner_ids).')');
            $res &= Db::getInstance()->execute('
    			DELETE FROM `'._DB_PREFIX_.'st_advanced_banner_lang`
    			WHERE `id_st_advanced_banner` in ('.implode(',',$banner_ids).')');
        }
        return $res;
    }
	public static function getCustomCss()
	{
		return  Db::getInstance()->executeS('
			SELECT `id_st_advanced_banner`, `description_color`, `btn_color`, `btn_bg`, `btn_hover_color`, `btn_hover_bg`
			FROM `'._DB_PREFIX_.'st_advanced_banner` 
