...
class StBlogSearchClass extends ObjectModel
{
	...
	{            
        ...
        foreach ($words as $key => $search)
        {
            ...
            INNER JOIN `'._DB_PREFIX_.'st_blog_shop` bs ON (bs.id_st_blog=bl.id_st_blog AND bl.`id_lang` ='.$id_lang.')
            LEFT JOIN `'._DB_PREFIX_.'st_blog_category_blog` bcb ON (bl.id_st_blog=bcb.id_st_blog)
            LEFT JOIN `'._DB_PREFIX_.'st_blog_category_lang` bcl ON (bcb.id_st_blog_category=bcl.id_lang AND bcl.`id_lang` = '.$id_lang.')
            WHERE bs.`id_shop` = '.$id_shop.'
