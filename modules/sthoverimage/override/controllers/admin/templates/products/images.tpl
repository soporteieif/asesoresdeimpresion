		...
		$(document).ready(function(){
			...
			{foreach from=$images item=image}
				...
				if (assoc != {literal}"{"{/literal})
				{
					...
					assoc = false;
				imageLine({$image->id}, "{$image->getExistingImgPath()}", {$image->position}, "{if $image->cover}icon-check-sign{else}icon-check-empty{/if}", assoc, "{$image->legend[$default_language]|escape:'htmlall'}","{if $image->hover}icon-check-square{else}icon-square-o{/if}");
			{/foreach}
			{literal}
