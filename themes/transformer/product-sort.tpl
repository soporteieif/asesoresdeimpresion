...
{if !isset($request)}
	...
	{/if}
{/if}
{assign var="category_sortby" value=trim(Configuration::get('STSN_CATE_SORTBY'),'¤')}
{if $category_sortby}
{assign var="category_sortby" value=explode('¤', $category_sortby)}
{else}
{assign var="category_sortby" value=''}
{/if}
...
<form id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" action="{$request|escape:'html':'UTF-8'}" class="productsSortForm">
	<div class="select selector1">
		...
		<select id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="selectProductSort form-control">
            ...
            {if $category_sortby}
    			{foreach $category_sortby AS $sortby}
                {if !$PS_CATALOG_MODE}
    				{if $sortby=='price_asc'}<option value="price:asc"{if $orderby eq 'price' AND $orderway eq 'asc'} selected="selected"{/if}>{l s='Price: Lowest first'}</option>{/if}
    				{if $sortby=='price_desc'}<option value="price:desc"{if $orderby eq 'price' AND $orderway eq 'desc'} selected="selected"{/if}>{l s='Price: Highest first'}</option>{/if}
    			{/if}
    			{if $sortby=='name_asc'}<option value="name:asc"{if $orderby eq 'name' AND $orderway eq 'asc'} selected="selected"{/if}>{l s='Product Name: A to Z'}</option>{/if}
    			{if $sortby=='name_desc'}<option value="name:desc"{if $orderby eq 'name' AND $orderway eq 'desc'} selected="selected"{/if}>{l s='Product Name: Z to A'}</option>{/if}
    			{if $PS_STOCK_MANAGEMENT && !$PS_CATALOG_MODE &&  $sortby=='quantity_desc'}
    				<option value="quantity:desc"{if $orderby eq 'quantity' AND $orderway eq 'desc'} selected="selected"{/if}>{l s='In stock'}</option>
    			{/if}
    			{if $sortby=='reference_asc'}<option value="reference:asc"{if $orderby eq 'reference' AND $orderway eq 'asc'} selected="selected"{/if}>{l s='Reference: Lowest first'}</option>{/if}
    			{if $sortby=='reference_desc'}<option value="reference:desc"{if $orderby eq 'reference' AND $orderway eq 'desc'} selected="selected"{/if}>{l s='Reference: Highest first'}</option>{/if}
                {/foreach}
            {else}
                {if !$PS_CATALOG_MODE}
