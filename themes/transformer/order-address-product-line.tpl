...
<tr id="product_{$product.id_product}_{$product.id_product_attribute}_0_{$product.id_address_delivery|intval}" class="{if $productLast}last_item{elseif $productFirst}first_item{/if} {if isset($customizedDatas.$productId.$productAttributeId) AND $quantityDisplayed == 0}alternate_item{/if} cart_item {if $odd}odd{else}even{/if}">
	...
	<td class="cart_ref" data-title="{l s='Ref.'}">
		<div class="mobile_table_content">
		{if $product.reference}{$product.reference|escape:'html':'UTF-8'}{else}--{/if}
		</div>
	</td>
    <td class="cart_avail" data-title="{l s='Avail.'}">
		<div class="mobile_table_content">
    	{if $product.stock_quantity > 0}<span class="label label-success">{l s='In Stock'}</span>{else}<span class="label label-warning">{l s='Out of Stock'}</span>{/if}
		</div>
    </td>
	<td class="cart_quantity {if isset($customizedDatas.$productId.$productAttributeId) AND $quantityDisplayed == 0} text-center {/if}" data-title="{l s='Qty'}">
		<div class="mobile_table_content">
	...
	<td data-title="{l s='Shipping address'}">
		<div class="mobile_table_content">
