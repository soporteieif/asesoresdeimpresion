...
{foreach $products as $product}
    ...
    {/if}
{/foreach}
<h1 class="page-heading">{l s='Address(es) Details'}</h1>
            ...
            {if $delivery->id}
                <div class="col-xs-12 col-sm-6"{if !$have_non_virtual_products} style="display: none;"{/if}>
                    <ul id="delivery_address" class="address item box">
                        ...
                        <li class="address_country">{$delivery->country|escape:'html':'UTF-8'} {if $delivery->id_state }{*there is a notice, hope it would be fixed soon({$delivery_state->name|escape:'html':'UTF-8'})*}{/if}</li>
                    </ul>
                </div>
            {/if}
            {if $invoice->id}
                <div class="col-xs-12 col-sm-6">
                    <ul id="invoice_address" class="address alternate_item box">
                        ...
                        <li class="address_country">{$invoice->country|escape:'html':'UTF-8'} {if $invoice->id_state}{*there is a notice, hope it would be fixed soon({$invoice->name|escape:'html':'UTF-8'})*}{/if}</li>
                    </ul>
                </div>
            {/if}
        {else}
