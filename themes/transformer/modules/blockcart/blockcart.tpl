...
<div id="blockcart_top_wrap" class="blockcart_wrap {if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}
		<a id="shopping_cart" href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow" class="shopping_cart clearfix">
				...
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
									...
									<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}" class="clearfix {if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										...
										<span class="remove_link">
											{if !isset($customizedDatas.$productId.$productAttributeId) && (!isset($product.is_gift) || !$product.is_gift)}
												<a class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_address_delivery={$product.id_address_delivery|intval}&token={$static_token}")|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='remove this product from my cart' mod='blockcart'}"><i class="icon-cancel icon-small"></i></a>
											{/if}
										</span>
										<span class="price">
										...
										<tr class="bloc_cart_voucher" data-id="bloc_cart_voucher_{$discount.id_discount|intval}">
											...
											<td class="delete">
												{if strlen($discount.code)}
													<a class="delete_voucher hidden" href="{$link->getPageLink("$order_process", true)}?deleteDiscount={$discount.id_discount|intval}" title="{l s='Delete' mod='blockcart'}" rel="nofollow">
						...
						<p class="cart-buttons">
							<a id="button_order_cart" class="btn btn-default" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">{l s='Check out' mod='blockcart'}</a>
						</p>
					</div>
				</div>
			</div><!-- .cart_block -->
			...
			<div class="layer_product clearfix mar_b10">
				...
				<div class="layer_product_info">
					...
					<div id="layer_cart_product_quantity_wrap">
						<span class="layer_cart_label">{l s='Quantity' mod='blockcart'}</span>
						<span id="layer_cart_product_quantity"></span>
					</div>
					<div id="layer_cart_product_price_wrap">
						<span class="layer_cart_label">{l s='Total' mod='blockcart'}</span>
						<span id="layer_cart_product_price"></span>
					</div>
				</div>
			</div>
			<div id="pro_added_success" class="success">{l s='Product successfully added to your shopping cart' mod='blockcart'}</div>
			<div class="layer_details">
				...
				{if $show_tax && isset($tax_cost)}
					<div id="layer_cart_ajax_cart_tax_cost" class="layer_cart_row hidden">
						<span class="layer_cart_label">{l s='Tax' mod='blockcart'}</span>
						<span class="price cart_block_tax_cost ajax_cart_tax_cost">{$tax_cost}</span>
					</div>
				{/if}
				...
				<div class="button-container clearfix">
					<span class="continue btn btn-default pull-left" title="{l s='Continue shopping' mod='blockcart'}">
						{l s='Continue shopping' mod='blockcart'}
					</span>
					<a class="btn btn-default pull-right" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow">
