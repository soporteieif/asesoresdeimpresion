			...
			{foreach from=$productsViewedObj item=viewedProduct name=myLoop}
				<li class="pro_column_box clearfix{if $smarty.foreach.myLoop.last} last_item{elseif $smarty.foreach.myLoop.first} first_item{else} item{/if}">
					...
					title="{l s='More about %s' mod='blockviewed' sprintf=[$viewedProduct->name|escape:'html':'UTF-8']}" >
						...
						src="{if isset($viewedProduct->id_image) && $viewedProduct->id_image}{$link->getImageLink($viewedProduct->link_rewrite, $viewedProduct->cover, 'thumb_default')}{else}{$img_prod_dir}{$lang_iso}-default-thumb_default.jpg{/if}" 
						alt="{$viewedProduct->legend|escape:'html':'UTF-8'}" 
						width="{$smarty.capture.thumb_default_width}" height="{$smarty.capture.thumb_default_height}" />
					</a>
