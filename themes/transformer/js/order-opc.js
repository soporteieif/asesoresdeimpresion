					...
					if (jsonData.hasError)
					{
						...
						$('#opc_account_errors').slideUp('fast', function(){
							$(this).html(errors).slideDown('slow', function(){
								$.scrollTo('#opc_account_errors', 800, {'offset': (((typeof(st_sticky_menu)!=='undefined' && st_sticky_menu) || (typeof(st_sticky_adv)!=='undefined' && st_sticky_adv)) ? -150 : -100) });
							});
						});
					}
					else
		...
		success: function(json)
		{
			...
			else
			{
				...
				$('#opc_new_account').fadeOut('fast', function() {
					...
					// 1.6 temmplate
					if (json.block_user_info_nav !== '' && $('#header .login').length == 1)
					{
						$('#header .sing_up, #header .welcome').remove();
						$('#header .login').fadeOut('normal', function() {
